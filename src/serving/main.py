...
def predict(customer: CustomerFeatures):
    ...
    try:
        ...
        churn_label = int(churn_prob > 0.446)  # Optimal threshold
        ...
        surv_features.columns = ['tenure', 'MonthlyCharges',
                                  'InternetService_encoded', 'Contract_encoded',
                                  'SeniorCitizen', 'service_count',
                                  'charge_per_month', 'digital_engagement']

        surv_7  = float(coxph_model.predict_survival_function(
            surv_features, times=[7]).values[0][0])
        surv_30 = float(coxph_model.predict_survival_function(
            surv_features, times=[30]).values[0][0])
        surv_90 = float(coxph_model.predict_survival_function(
            surv_features, times=[90]).values[0][0])
...
def explain(customer: CustomerFeatures):
    """SHAP explanation — tại sao model dự đoán vậy"""
    try:
        data = pd.DataFrame([customer.dict()])[feature_cols]
...
def segment(customer: UpliftFeatures):
    """Phân nhóm Uplift segment + recommend action"""
    try:
        data = pd.DataFrame([customer.dict()])
        ...
        # Percentile-based segment (dùng fixed thresholds làm proxy)
